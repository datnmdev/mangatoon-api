...
export class JwtMiddleware {
    ...
    static verifyRefreshToken =  async (req: Request, res: Response, next: NextFunction) => {
        try {
            const refreshToken = (req.headers?.['authorization'] as string)?.split('Bearer ')?.[1]
            const user = jwt.verify(refreshToken, envVariables.REDIS_REFRESH_TOKEN_SECRET) as Payload
            ...
            const refreshTokenInRedis = await redisClient.get(RedisKeyGenerator.refreshTokenkey(refreshToken))
